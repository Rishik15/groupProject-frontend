import { Button } from "@heroui/react";

interface GoogleAuthButtonProps {
    onPress: () => void;
    label?: string;
}

const GoogleIcon = () => (
    <svg width="18" height="18" viewBox="0 0 48 48" aria-hidden="true">
        <path
            fill="#FFC107"
            d="M43.6 20.5H42V20H24v8h11.3C33.6 32.7 29.2 36 24 36c-6.6 0-12-5.4-12-12s5.4-12 12-12c3 0 5.7 1.1 7.8 3l5.7-5.7C34.1 6.1 29.3 4 24 4 12.9 4 4 12.9 4 24s8.9 20 20 20 20-8.9 20-20c0-1.3-.1-2.3-.4-3.5Z"
        />
        <path
            fill="#FF3D00"
            d="M6.3 14.7l6.6 4.8C14.7 15.1 18.9 12 24 12c3 0 5.7 1.1 7.8 3l5.7-5.7C34.1 6.1 29.3 4 24 4c-7.7 0-14.3 4.3-17.7 10.7Z"
        />
        <path
            fill="#4CAF50"
            d="M24 44c5.2 0 10-2 13.5-5.2l-6.2-5.2c-2.1 1.6-4.7 2.4-7.3 2.4-5.2 0-9.6-3.3-11.2-8l-6.5 5C9.6 39.5 16.3 44 24 44Z"
        />
        <path
            fill="#1976D2"
            d="M43.6 20.5H42V20H24v8h11.3c-.8 2.3-2.2 4.2-4 5.6l6.2 5.2C36.9 38.4 44 33 44 24c0-1.3-.1-2.3-.4-3.5Z"
        />
    </svg>
);

const GoogleAuthButton = ({
    onPress,
    label = "Continue with Google",
}: GoogleAuthButtonProps) => {
    return (
        <Button
            onPress={onPress}
            variant="outline"
            className="w-full h-12 bg-white border border-neutral-300 text-neutral-800 font-medium"
        >
            <span className="flex items-center justify-center gap-3">
                <GoogleIcon />
                <span>{label}</span>
            </span>
        </Button>
    );
};

export default GoogleAuthButton;