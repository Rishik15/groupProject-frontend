...
const MetricPlots = ({
  ...
  contract_id: number;
  onTabChange: (tab: "dashboard" | "workouts" | "nutrition") => void;
}) => {
  ...
  useEffect(() => {
    const fetchWeight = async () => {
      try {
        const res = await getWeight(contract_id);
    ...
    const fetchNutrition = async () => {
      const nutritionData = await getNutrition(contract_id);
  ...
  return (
    <section className="flex gap-6 px-12">
