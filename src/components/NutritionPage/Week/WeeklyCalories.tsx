...
import { getWeeklyCaloriesSummary } from "../../../services/Nutrition/nutritionWeekService";
...
const WeeklyCalories = () => {
    ...
    useEffect(() => {
        const fetchWeeklyData = async () => {
            try {
                const data = await getWeeklyCaloriesSummary();
                setSummary(data);
            } catch (error) {
                    ...
                    <div className="grid grid-cols-[48px_1fr] gap-2">
                        <div className="relative h-[320px] text-[12px] text-[#72728A]">
                            {[2400, 1800, 1200, 600, 0].map((value, index) => (
                                <div
                                    key={value}
                                    className={`absolute right-0 w-full text-right ${index === 0
                                        ? "top-0"
                                        : index === 1
                                            ? "top-1/4"
                                            : index === 2
