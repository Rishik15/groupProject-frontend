...
// represents a selected exercise with sets and reps
  ...
  if (selected.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-12 text-center">
        ...
        <p className="text-sm font-medium text-black">No exercises added yet</p>
        <p className="text-xs text-[#72728A] mt-1">Browse and add exercises from the left</p>
      </div>
    );
  }
        ...
        >
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-xs text-[#72728A]">{exercise.category} · {exercise.muscle_group}</p>
            </div>
            <button
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              ...
              <input
                ...
                max={10}
                value={sets}
                onChange={(e) => onUpdateSets(exercise.id, Number(e.target.value))}
            ...
            <div className="flex items-center gap-2">
              ...
              <input
                ...
                max={100}
                value={reps}
                onChange={(e) => onUpdateReps(exercise.id, Number(e.target.value))}
                className="w-14 text-center text-sm font-medium border border-[#E6E6EE] rounded-lg py-1 focus:outline-none focus:border-[#5B5EF4]"
              />
            </div>
          </div>
