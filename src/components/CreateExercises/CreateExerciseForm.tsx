...
function FormField({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="flex flex-col gap-1.5">
      <label className="text-xs font-semibold text-[#72728A] uppercase tracking-wider">{label}</label>
      {children}
    </div>
  );
}
  ...
  return (
    <div className="flex flex-col gap-6 max-w-2xl w-full mx-auto">
      ...
      <FormField label="Exercise Name">
        <input type="text" placeholder="e.g. Bulgarian Split Squat" value={name}
      ...
      <FormField label="Description">
        <textarea placeholder="Describe how to perform this exercise..." value={description}
      ...
      <FormField label="Exercise Video (optional)">
        <div className="w-full border border-dashed border-[#E6E6EE] rounded-xl px-4 py-6 flex flex-col items-center gap-2 cursor-pointer hover:border-[#5B5EF4] transition-colors"
        ...
        {video && <button onClick={() => setVideo(null)} className="text-xs text-red-400 hover:text-red-600 self-start">Remove video</button>}
      </FormField>
