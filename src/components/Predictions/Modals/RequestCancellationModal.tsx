...
type RequestCancellationModalProps = {
    ...
    error?: string | null;
    onClose: () => void;
    onSubmit: (payload: RequestPredictionCancellationPayload) => void | Promise<void>;
};
        ...
        <Modal>
            ...
            >
                <Modal.Container placement="center" size="md" scroll="inside">
                    <Modal.Dialog aria-label="Request market cancellation" className="rounded-3xl">
                        <Modal.Header className="flex items-start justify-between gap-4 border-b border-slate-100 px-6 py-5">
                            <div className="space-y-1">
                                ...
                                <h2 className="text-[18.75px] font-semibold text-slate-900">{market?.title ?? "Request cancellation"}</h2>
                                <p className="text-[13.125px] text-slate-500">
                                    This sends the market to admin cancel review and should only be used when necessary.
                                </p>
                            </div>
                        </Modal.Header>
                        ...
                        <Modal.Body className="space-y-5 px-6 py-6">
                            <div className="space-y-2">
                                <label htmlFor="prediction-cancel-reason" className="text-[13.125px] font-medium text-slate-700">
                                    Cancellation reason
                                </label>
                        ...
                        <Modal.Footer className="flex gap-3 border-t border-slate-100 px-10 py-2">
                            ...
                            <Button variant="danger" onPress={handleSubmit} isDisabled={isSubmitting || !market}>
                                <span className="inline-flex items-center gap-2">
                                    {isSubmitting ? <Spinner size="sm" /> : <AlertTriangle className="h-4 w-4" />}
                                    Send for review
                                </span>
                            </Button>
                        </Modal.Footer>
