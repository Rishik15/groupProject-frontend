...
export default function ProfileHeader({ coach, coachId }: ProfileHeaderProps) {
  ...
  const canRequest = useMemo(() => {
    return contractStatus === "none" || contractStatus === "closed";
  }, [contractStatus]);
  ...
  const buttonText = (() => {
    ...
    if (contractStatus === "active") return "Already Coaching";
    if (contractStatus === "pending") return "Request Pending";
    return "Request Coaching";
  })();

  const buttonClassName = (() => {
    if (contractStatus === "active") {
      return "bg-default-100 text-default-500 border border-default-200";
    }
    ...
    if (isSubmitting || isCheckingStatus) {
      return "bg-[#5B5EF4]/70 text-white";
    }

    return "bg-[#5B5EF4] text-white hover:bg-[#4B4EE4]";
  })();

  const showCheckIcon = contractStatus === "pending";
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex gap-3">
        <button
          ...
          disabled={isCheckingStatus || isSubmitting || !canRequest}
          className={`flex-1 flex items-center justify-center gap-2 text-sm font-medium py-2.5 rounded-xl transition-colors disabled:cursor-not-allowed ${buttonClassName}`}
        >
          {showCheckIcon && (
