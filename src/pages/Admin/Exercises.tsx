...
const exerciseTabs: Array<{
    ...
    description: string;
}> = [
        {
            ...
            icon: ClipboardList,
            description: "Create, edit, search, and remove exercises using the admin exercise CRUD endpoints.",
        },
        {
            key: "video-moderation",
            label: "Video Moderation",
            icon: PlaySquare,
            description: "Review pending exercise videos and approve, reject, or remove clips from the moderation queue.",
        },
    ];
...
const Exercises = () => {
    ...
    const currentTab = exerciseTabs.find((tab) => tab.key === selectedTab) ?? exerciseTabs[0];
        ...
        <div>
            <div className="justify-between items-center px-38 py-4 bg-white shadow-[0_1px_2px_rgba(0,0,0,0.05)]">
                <div>
                    ...
                    <p className="mt-2 max-w-3xl text-[13.125px]">
                        Manage exercise records and moderate uploaded exercise videos from one fixed admin page.
                    </p>
                </div>
            </div>
            <div className="min-h-[calc(100vh-56px)] bg-default-50 px-36 py-8">
                <div className="space-y-6">
                    ...
                    {selectedTab === "management" ? <ExerciseManagementTab /> : <VideoModerationTab />}
                </div>
            </div>
        </div>
    );
