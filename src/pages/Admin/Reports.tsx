...
const reportTabs: Array<{
...
}> = [
  {
    ...
    icon: FileWarning,
    description:
      "Review reports awaiting moderation and close them with an admin note.",
  },
  {
    key: "closed",
    ...
    icon: ClipboardList,
    description:
      "Review previous moderation outcomes and stored admin action history.",
  },
];
...
const Reports = () => {
  ...
  const currentTab =
    reportTabs.find((tab) => tab.key === selectedTab) ?? reportTabs[0];
      ...
      <div className="justify-between items-center px-38 py-4 bg-white shadow-[0_1px_2px_rgba(0,0,0,0.05)]">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-[13.125px] text-default-600">
            ...
            reference closed report history.
          </p>
        </div>
      </div>
