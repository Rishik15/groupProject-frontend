...
export default function Predictions() {
  ...
  const [settlementError, setSettlementError] = React.useState<string | null>(
    null,
  );
  ...
  const [cancelReviewMode, setCancelReviewMode] = React.useState<
    "approve" | "reject"
  >("approve");
  const [cancelReviewError, setCancelReviewError] = React.useState<
    string | null
  >(null);
  ...
  const loadQueues = React.useCallback(async (showLoader: boolean) => {
    ...
    try {
      const [reviewQueue, settlementQueue, cancelQueue] = await Promise.all([
        getMarketsInReview(),
        getPendingSettlementMarkets(),
        getCancellationReviewMarkets(),
      ]);
  ...
  const summaryCards = React.useMemo(
    () => [
      ...
      },
      {
        ...
        icon: ClipboardCheck,
        description: "Closed approved markets awaiting final resolution.",
      },
      {
        id: "cancel",
        title: "Cancel Review",
        ...
        icon: ShieldAlert,
        description: "Markets with pending creator cancellation requests.",
      },
    ],
    [queueSummary],
  );
  ...
  const handleReject = async (market: PredictionMarket) => {
    ...
    try {
      ...
      const message =
        error instanceof Error ? error.message : "Failed to reject market.";
      setPageError(message);
    } finally {
      setPendingMarketId(null);
    }
      ...
      <div className="min-h-screen bg-slate-50 px-36 py-4">
        <div className="mx-auto flex max-w-7xl flex-col gap-8">
          <PredictionAdminContent
            ...
            onRejectCancel={(market) => openCancelReviewModal(market, "reject")}
          />
        </div>
