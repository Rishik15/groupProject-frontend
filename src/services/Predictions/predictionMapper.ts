...
import type { PredictionBet, PredictionBetResult } from "../../utils/Interfaces/Predictions/predictionBet";
...
const REVIEW_STATUSES = new Set<PredictionReviewStatus>(["approved", "pending", "rejected"]);
const MARKET_STATUSES = new Set<PredictionMarketStatus>(["open", "closed", "settled", "cancelled"]);
const CANCEL_REQUEST_STATUSES = new Set<PredictionCancelRequestStatus>(["none", "pending", "approved", "rejected"]);
const BET_VALUES = new Set(["yes", "no"] as const);
const SETTLEMENT_RESULTS = new Set<Exclude<PredictionSettlementResult, null>>(["yes", "no", "cancelled"]);
const BET_RESULTS = new Set<Exclude<PredictionBetResult, null>>(["won", "lost", "refunded", "pending"]);
...
const asNullableString = (value: unknown): string | null => asString(value) ?? null;
...
const asNullableNumber = (value: unknown): number | null => asNumber(value) ?? null;

const asEnum = <T extends string>(value: unknown, validValues: Set<T>): T | undefined => {
...
export const normalizeReviewStatus = (value: unknown): PredictionReviewStatus =>
  asEnum(value, REVIEW_STATUSES) ?? "pending";

export const normalizeMarketStatus = (value: unknown): PredictionMarketStatus =>
  asEnum(value, MARKET_STATUSES) ?? "open";

export const normalizeCancelRequestStatus = (
  value: unknown,
): PredictionCancelRequestStatus => asEnum(value, CANCEL_REQUEST_STATUSES) ?? "none";

export const normalizePredictionValue = (value: unknown): "yes" | "no" =>
  asEnum(value, BET_VALUES as Set<"yes" | "no">) ?? "yes";

export const normalizeSettlementResult = (value: unknown): PredictionSettlementResult =>
  asEnum(value, SETTLEMENT_RESULTS) ?? null;
...
export const mapPredictionMarket = (raw: unknown): PredictionMarket => {
  ...
  return {
    ...
    cancel_request_status: normalizeCancelRequestStatus(record.cancel_request_status),
    ...
    cancel_reviewed_by_admin_id: asNullableNumber(record.cancel_reviewed_by_admin_id),
...
export const mapPredictionSummary = (raw: unknown): PredictionSummary => {
  ...
  return {
    ...
    completed_markets_participated: asNumber(record.completed_markets_participated) ?? 0,
  };
};
...
export const mapPredictionWalletTransaction = (raw: unknown): PredictionWalletTransaction => {
  const record = asRecord(raw);
...
export const mapPredictionWalletTransactions = (raw: unknown): PredictionWalletTransaction[] =>
  Array.isArray(raw) ? raw.map(mapPredictionWalletTransaction) : [];

export const mapPredictionLeaderboardEntry = (raw: unknown): PredictionLeaderboardEntry => {
  const record = asRecord(raw);
...
export const mapPredictionLeaderboard = (raw: unknown): PredictionLeaderboardEntry[] =>
...
export const mapAdminPrediction = (raw: unknown): AdminPrediction => {
  ...
  return {
    ...
    review_status: normalizeReviewStatus(record.review_status),
    cancel_request_status: normalizeCancelRequestStatus(record.cancel_request_status),
    settlement_result: normalizeSettlementResult(record.settlement_result ?? record.result),
    total_bets: asNumber(record.total_bets) ?? 0,
    total_points: asNumber(record.total_points) ?? 0,
    review_note: asNullableString(record.review_note),
    settlement_note: asNullableString(record.settlement_note),
    cancel_request_reason: asNullableString(record.cancel_request_reason ?? record.cancel_reason),
    cancel_review_note: asNullableString(record.cancel_review_note),
    created_at: asNullableString(record.created_at),
    updated_at: asNullableString(record.updated_at),
  };
