...
export const getPredictionSummary = async (): Promise<PredictionSummary> => {
    const { data } = await api.get<ApiEnvelope<{ summary: unknown }>>(
        "/predictions/summary",
        withCredentials,
    );
...
export const getOpenMarkets = async (): Promise<PredictionMarket[]> => {
    const { data } = await api.get<ApiEnvelope<{ markets: unknown[] }>>(
        "/predictions/markets/open",
        withCredentials,
    );
...
export const getLeaderboard = async (): Promise<PredictionLeaderboardEntry[]> => {
    const { data } = await api.get<ApiEnvelope<{ leaderboard: unknown[] }>>(
...
export const getMyBets = async (): Promise<PredictionBet[]> => {
    const { data } = await api.get<ApiEnvelope<{ bets: unknown[] }>>(
        "/predictions/me/bets",
        withCredentials,
    );
...
export const createMarket = async (
    payload: CreatePredictionMarketPayload,
): Promise<PredictionMarket> => {
    const { data } = await api.post<ApiEnvelope<{ market: unknown }>>(
        "/predictions/markets",
        payload,
        {
            ...withCredentials,
...
export const placeBet = async (
    payload: PlacePredictionBetPayload,
): Promise<PredictionBet> => {
    const { data } = await api.post<ApiEnvelope<{ bet: unknown }>>(
        "/predictions/bets",
        payload,
        {
            ...withCredentials,
...
export const closeMarket = async (
    payload: ClosePredictionMarketPayload,
): Promise<PredictionMarket> => {
    const { data } = await api.patch<ApiEnvelope<{ market: unknown }>>(
        "/predictions/markets/close",
        payload,
        {
            ...withCredentials,
            headers: JSON_HEADERS,
...
export const requestMarketCancellation = async (
    ...
    const { data } = await api.patch<
        ApiEnvelope<{ cancel_request: CancelRequestResponse }>
    >("/predictions/markets/cancel-request", payload, {
        ...withCredentials,
        headers: JSON_HEADERS,
    });
