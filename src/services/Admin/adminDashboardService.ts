...
export async function getDashboardStats() {
  const { data } = await api.get<StatsResponse>("/admin/dashboard/stats");
  return data.stats;
}
...
export async function getActiveCoaches() {
  const { data } = await api.get<CoachesResponse>("/admin/coaches/active");
  return data.coaches;
}
...
export async function getCoachApplications(
  ...
  const { data } = await api.get<CoachApplicationsResponse>(
...
export async function approveCoachApplication(
  ...
  const { data } = await api.patch<CoachApplicationMutationResponse>(
    `/admin/coach-applications/${applicationId}/approve`,
...
export async function rejectCoachApplication(
  ...
  const { data } = await api.patch<CoachApplicationMutationResponse>(
    `/admin/coach-applications/${applicationId}/reject`,
    { admin_action: adminAction },
  );
...
export async function closeAdminReport(reportId: number, adminAction?: string) {
  const { data } = await api.patch<ReportMutationResponse>(
